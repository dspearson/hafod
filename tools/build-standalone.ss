...
(let* ([solibs (if (threaded?) "-ldl -lm -llz4 -lz -lncurses -lpthread" "-ldl -lm -llz4 -lz -lncurses")]
       [ldflags (or (getenv "LDFLAGS") "")]
       [cc (or (getenv "CC") "cc")]
       [cflags (or (getenv "CFLAGS") "")]
       [cmd (format "~a -O2 -o ~a/hafod-standalone ~a/hafod-standalone.c ~a/boot_data.c ~a/hafod_boot_data.c ~a/prog_data.c ~a/libkernel.a -I~a ~a ~a ~a -m64 -Wno-format-truncation"
