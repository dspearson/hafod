...
(define (bench name n thunk)
  (let ([t0 (current-time 'time-monotonic)])
    ...
    (let* ([t1 (current-time 'time-monotonic)]
           [ds (- (time-second t1) (time-second t0))]
           [dns (- (time-nanosecond t1) (time-nanosecond t0))]
           [ms (+ (* ds 1000) (/ dns 1000000))])
      (format #t "BENCH ~a ~a~%" name (exact->inexact ms)))))
